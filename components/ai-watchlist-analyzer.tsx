...
export default function AIWatchlistAnalyzer({ 
  ...
  const fetchAdvancedTechnicalAnalysis = useCallback(async (symbol: string) => {
    try {
      ...
      // Try to get result by exact symbol match first, then try without .NS suffix
      let result = data.results?.[symbol]
      if (!result && symbol.endsWith('.NS')) {
        const baseSymbol = symbol.replace('.NS', '')
        result = data.results?.[baseSymbol]
        console.log(`Fallback: trying base symbol ${baseSymbol}, result:`, result)
      }
